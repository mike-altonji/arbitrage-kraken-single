...
pub async fn run_trading_thread(
    ...
    // For LimitIoc mode, we need to listen to ownTrades messages
    let (filled_volume_tx, mut filled_volume_rx) = if trade_mode == TradeMode::LimitIoc {
        ...
        // For Market mode, we don't need the read stream
        drop(read);
        (None, None)
    };
    ...
    while let Some(order) = trade_rx.recv().await {
        ...
        if allow_trades {
            log::debug!(
                "Sending order starting with {} (mode: {:?})",
                order.pair1_name,
                trade_mode
            );
            match trade_mode {
                ...
                TradeMode::LimitIoc => {
                    if let (Some(_tx), Some(rx)) = (&filled_volume_tx, &mut filled_volume_rx) {
                        make_trades_limit_ioc(&mut write, &token, &order, rx).await;
                    } else {
                        log::error!(
                            "LimitIoc mode requires ownTrades listener but it's not available"
                        );
                    }
                }
            }
